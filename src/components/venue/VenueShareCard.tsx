...
const VenueShareCard = ({
  name, venueType, city, country, capacity, hourlyRate, minHours, image,
}: VenueShareCardProps) => {
  ...
  const downloadCard = async () => {
    ...
    try {
      ...
      link.download = `${name.replace(/\s+/g, "-").toLowerCase()}-venue.png`;
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-sm">
        ...
        <div ref={cardRef} className="rounded-2xl overflow-hidden bg-card border border-border/50">
          <div className="h-40 relative overflow-hidden">
            ...
            <div className="absolute bottom-3 left-3 right-3">
              ...
              <p className="text-white/80 text-xs mt-0.5">{formatType(venueType)}</p>
            </div>
          </div>
          <div className="p-4 space-y-2">
            ...
            <div className="flex items-center gap-4 text-sm">
              ...
              <span className="flex items-center gap-1 text-muted-foreground">
                <Clock className="h-3.5 w-3.5" /> Min {minHours}h
              </span>
            </div>
            {hourlyRate && (
        ...
        <div className="flex gap-2 mt-2">
          <Button onClick={shareWhatsApp} className="flex-1 rounded-full press-effect gap-1.5 bg-green-600 hover:bg-green-700">
