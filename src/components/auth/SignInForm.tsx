...
import { Loader2, Mail, Phone, Fingerprint } from "lucide-react";
import type { AuthMethod } from "./useAuthFlow";

interface SignInFormProps {
  authMethod: AuthMethod;
  setAuthMethod: (m: AuthMethod) => void;
  formData: { email: string; password: string; phone: string };
...
export const SignInForm = ({
  ...
  onSubmit, onGoogleSignIn, onForgotPassword, onToggleMode,
}: SignInFormProps) => {
  return (
    <>
      {/* Header */}
      <div className="text-center mb-8">
        ...
        >
          <div className="absolute inset-0 bg-gradient-to-br from-primary/40 to-primary/10 rounded-3xl blur-xl" />
      ...
      {/* Auth Method Toggle */}
      <motion.div
        className="flex gap-1 p-1 bg-muted/60 rounded-2xl mb-6 backdrop-blur-sm"
        initial={{ opacity: 0, y: 10 }}
        animate={{ opacity: 1, y: 0 }}
      ...
      <motion.div className="mt-8 text-center space-y-3" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }}>
        ...
        <p className="text-xs text-muted-foreground/60 leading-relaxed">
          ...
          <a href="/terms" target="_blank" className="text-primary/70 underline underline-offset-2 hover:text-primary">Terms & Conditions</a>{" "}
          and{" "}
          <a href="/privacy" target="_blank" className="text-primary/70 underline underline-offset-2 hover:text-primary">Privacy Policy</a>.
        </p>
      </motion.div>
    </>
  );
