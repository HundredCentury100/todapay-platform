...
const SettingsPage = () => {
  const { merchantProfile } = useMerchantAuth("event_organizer");
  ...
  const handleAddOrganizer = async () => {
    if (!merchantProfile || !newOrganizer.trim()) return;

    try {
      await addOperatorAssociation(merchantProfile.id, newOrganizer.trim());
      setNewOrganizer("");
      await loadOrganizers();
      toast({
        title: "Organizer Added",
        description: "Event organizer association added successfully",
      });
    } catch (error) {
    ...
    <div className="space-y-6">
      ...
      <Tabs defaultValue="profile" className="space-y-6">
          ...
          <div className="grid gap-6 md:grid-cols-2">
            <Card className="p-6">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                  <Label htmlFor="business_email">Organization Email *</Label>
                  <Input
                    id="business_email"
                    type="email"
                ...
                <div>
                  <Label htmlFor="business_address">Organization Address</Label>
                  <Textarea
                    id="business_address"
            ...
            <Card className="p-6">
              ...
              <p className="text-sm text-muted-foreground mb-4">
                Link your account to event organizer names in the system
              </p>
              ...
              <div className="space-y-4">
                <div className="flex gap-2">
                  ...
                  />
                  <Button onClick={handleAddOrganizer} disabled={!newOrganizer.trim()}>
                    <Plus className="w-4 h-4" />
                  </Button>
                </div>

                <div className="space-y-2">
                  <Label>Associated Organizers</Label>
                  <div className="flex flex-wrap gap-2">
                    {organizers.length === 0 ? (
                      <p className="text-sm text-muted-foreground">No organizers associated yet</p>
                    ) : (
                      organizers.map((org) => (
        ...
        <TabsContent value="policy">
          {merchantProfile && (
            <BookingPolicyManager policyType="event" merchantProfileId={merchantProfile.id} />
          )}
        </TabsContent>
      </Tabs>
    </div>
