...
const ReviewsPage = () => {
  ...
  const loadReviews = async () => {
    try {
      ...
      setReviews((data as unknown as EventReview[]) || []);
    } catch (error) {
  ...
  const handleRespond = (review: EventReview) => {
      ...
      setReviews((prev) =>
        prev.map((r) =>
          r.id === selectedReview.id
            ? { ...r, merchant_response: response.trim(), responded_at: new Date().toISOString() }
            : r
        )
      );
      toast({ title: "Response saved", description: "Your response has been published." });
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold mb-2">Event Reviews & Ratings</h1>
        <p className="text-muted-foreground">Manage attendee feedback and reviews</p>
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-3">
        <Card className="p-6 md:col-span-2">
          <h2 className="text-xl font-semibold mb-4">Attendee Reviews</h2>
          {loading ? (
            ...
            <div className="space-y-4">
              {reviews.map((review) => (
                <div key={review.id} className="p-4 border rounded-lg">
                  ...
                  <Button size="sm" variant="outline" onClick={() => handleRespond(review)}>
      ...
      <Dialog open={!!selectedReview} onOpenChange={() => setSelectedReview(null)}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Write a response to this attendee's review</DialogDescription>
          </DialogHeader>
          {selectedReview && (
            <div className="space-y-4">
